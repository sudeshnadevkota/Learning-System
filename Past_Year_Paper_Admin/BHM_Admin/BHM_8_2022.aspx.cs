...
{
    public partial class BHM_8_2022 : System.Web.UI.Page
    {
        ...
        protected void Button1_Click(object sender, EventArgs e)
        {
            ...
            using (Stream fs = FileUpload1.PostedFile.InputStream)
            {
                using (BinaryReader br = new BinaryReader(fs))
                {
                    ...
                    {
                        string query = "INSERT INTO BHM_8_2022 (Topic, Name, ContentType, Data) VALUES (@Topic, @Name, @ContentType, @Data)";
        ...
        private void BindGrid()
        {
            using (SqlConnection con = new SqlConnection(strcon))
            {
                using (SqlCommand cmd = new SqlCommand("SELECT * FROM BHM_8_2022", con))
                {
                    con.Open();
        ...
        private void UpdateRecord(int id, string topic, string name, string contentType, byte[] data)
        {
            using (SqlConnection con = new SqlConnection(strcon))
            {
                string query = "UPDATE BHM_8_2022 SET Topic = @Topic, Name = @Name, ContentType = @ContentType, Data = @Data WHERE Id = @Id";
                using (SqlCommand cmd = new SqlCommand(query, con))
                {
        ...
        private void DeleteRecord(int id)
        {
            using (SqlConnection con = new SqlConnection(strcon))
            {
                string query = "DELETE FROM BHM_8_2022 WHERE Id = @Id";
                using (SqlCommand cmd = new SqlCommand(query, con))
                {
                    cmd.Parameters.AddWithValue("@Id", id);
