    ...
    public partial class computer_network : System.Web.UI.Page
    {
        ...
        protected void DownloadFile(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    using (SqlCommand cmd = new SqlCommand("SELECT Name, Data, ContentType FROM bit_1_CN WHERE Id = @Id", con))
                    {
        ...
        private void BindGrid()
        {
            ...
            using (SqlConnection con = new SqlConnection(constr))
            {
                using (SqlCommand cmd = new SqlCommand("SELECT Id, Name, Topic FROM bit_1_CN", con))
                {
                    con.Open();
