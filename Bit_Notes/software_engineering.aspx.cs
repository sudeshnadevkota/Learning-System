...
namespace Learning_System.Bit_Notes
{
    public partial class software_engineering : System.Web.UI.Page
    {
        ...
        protected void DownloadFile(object sender, EventArgs e)
        {
            try
            {
                ...
                if (int.TryParse((sender as LinkButton)?.CommandArgument, out int id))
                {
                    ...
                    {
                        using (SqlCommand cmd = new SqlCommand("SELECT Name, Data, ContentType FROM bit_4_SE WHERE Id = @Id", con))
                        {
        ...
        private void BindGrid()
        {
            ...
            using (SqlConnection con = new SqlConnection(constr))
            {
                using (SqlCommand cmd = new SqlCommand("SELECT Id, Name, Topic FROM bit_4_SE", con))
                {
                    con.Open();
