...
namespace Learning_System.Bit_Notes
{
    public partial class operating_system : System.Web.UI.Page
    {
        ...
        protected void DownloadFile(object sender, EventArgs e)
        {
            try
            {
                ...
                if (int.TryParse((sender as LinkButton)?.CommandArgument, out int id))
                {
                    ...
                    {
                        using (SqlCommand cmd = new SqlCommand("SELECT Name, Data, ContentType, Topic FROM bit_3_OS WHERE Id = @Id", con))
                        {
        ...
        private void BindGrid()
        {
            ...
            using (SqlConnection con = new SqlConnection(constr))
            {
                using (SqlCommand cmd = new SqlCommand("SELECT Id, Name, Topic FROM bit_3_OS", con))
                {
                    con.Open();
