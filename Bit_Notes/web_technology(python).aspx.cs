...
namespace Learning_System.Bit_Notes
{
    public partial class web_technology_python_ : System.Web.UI.Page
    {
        ...
        protected void DownloadFile(object sender, EventArgs e)
        {
            try
            {
                ...
                if (int.TryParse((sender as LinkButton)?.CommandArgument, out int id))
                {
                    ...
                    {
                        using (SqlCommand cmd = new SqlCommand("SELECT Name, Data, ContentType FROM bit_3_WTP WHERE Id = @Id", con))
                        {
                            ...
                            using (SqlDataReader sdr = cmd.ExecuteReader())
                            {
                                ...
                                {
                                    // File not found, exit the method
            ...
            {
                // Handle exception and log the error for diagnostics
        ...
        private void BindGrid()
        {
            ...
            using (SqlConnection con = new SqlConnection(constr))
            {
                using (SqlCommand cmd = new SqlCommand("SELECT Id, Name, Topic FROM bit_3_WTP", con))
                {
                    con.Open();
