...
{
    public partial class business_communications : System.Web.UI.Page
    {
        ...
        protected void Button1_Click(object sender, EventArgs e)
        {
            ...
            using (Stream fs = FileUpload1.PostedFile.InputStream)
            {
...

                {
                    ...
                    {
                        string query = "insert into bba_bc(Topic,Name,ContentType,Data)values(@Topic, @Name, @ContentType, @Data)";
        ...
        private void BindGrid()
        {
            ...
            using (SqlConnection con = new SqlConnection(strcon))
            {
                ...
                {
                    cmd.CommandText = "select * from bba_bc";
                    cmd.Connection = con;
                    con.Open();
        ...
        private void update(int id, string topic, string name, string content, string data)
        {
            using (SqlConnection con = new SqlConnection(strcon))
            {
                string sqlquery = "update bba_bc set Topic = '" + topic + "',Name='" + name + "',ContentType='" + content + "',Data='" + data + "' where id=" + id + "";
        ...
        private void delete(int id)
        {
            using (SqlConnection con = new SqlConnection(strcon))
            {
                string sqlquery = "delete from bba_bc where id=" + id + "";
                SqlCommand cmd = new SqlCommand(sqlquery, con);
                con.Open();
                cmd.ExecuteNonQuery();
            }
