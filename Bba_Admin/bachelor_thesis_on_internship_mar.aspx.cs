...
{
    public partial class bachelor_thesis_on_internship_mar : Page
    {
        ...
        protected void Button1_Click(object sender, EventArgs e)
        {
            if (FileUpload1.HasFile)
            {
                ...
                using (BinaryReader br = new BinaryReader(fs))
                {
                    ...
                    {
                        string query = "INSERT INTO bba_bti_mark (Topic, Name, ContentType, Data) VALUES (@Topic, @Name, @ContentType, @Data)";
        ...
        private void BindGrid()
        {
            using (SqlConnection con = new SqlConnection(strcon))
            {
                string query = "SELECT * FROM bba_bti_mark";
                using (SqlCommand cmd = new SqlCommand(query, con))
                {
                    con.Open();
        ...
        private void UpdateRecord(int id, string topic, string name, string contentType, byte[] data)
        {
            using (SqlConnection con = new SqlConnection(strcon))
            {
                string query = "UPDATE bba_bti_mark SET Topic = @Topic, Name = @Name, ContentType = @ContentType, Data = @Data WHERE id = @Id";
                using (SqlCommand cmd = new SqlCommand(query, con))
                {
                    cmd.Parameters.AddWithValue("@Topic", topic);
        ...
        private void DeleteRecord(int id)
        {
            using (SqlConnection con = new SqlConnection(strcon))
            {
                string query = "DELETE FROM bba_bti_mark WHERE id = @Id";
                using (SqlCommand cmd = new SqlCommand(query, con))
                {
                    cmd.Parameters.AddWithValue("@Id", id);
        ...
        protected void GridView1_RowUpdating1(object sender, GridViewUpdateEventArgs e)
        {
            ...
            // Convert TextBox text to byte array for Data field if needed
            byte[] data = Convert.FromBase64String(txtData.Text);

            UpdateRecord(id, txtTopic.Text, txtName.Text, txtContent.Text, data);
