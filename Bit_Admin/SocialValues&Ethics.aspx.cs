...
{
    public partial class SocialValues_Ethics : System.Web.UI.Page
    {
            ...
            if (FileUpload1.HasFile)
            {
                using (Stream fs = FileUpload1.PostedFile.InputStream)
                {
                    using (BinaryReader br = new BinaryReader(fs))
                    {
                        ...
                        {
                            string query = "INSERT INTO bit_sve (Topic, Name, ContentType, Data) VALUES (@Topic, @Name, @ContentType, @Data)";
        ...
        private void BindGrid()
        {
            using (SqlConnection con = new SqlConnection(strcon))
            {
                using (SqlCommand cmd = new SqlCommand("SELECT * FROM bit_sve", con))
                {
                    con.Open();
        ...
        private void UpdateRecord(int id, string topic, string name, string contentType, string data)
        {
            using (SqlConnection con = new SqlConnection(strcon))
            {
                string query = "UPDATE bit_sve SET Topic = @Topic, Name = @Name, ContentType = @ContentType, Data = @Data WHERE id = @Id";
                using (SqlCommand cmd = new SqlCommand(query, con))
                {
                    cmd.Parameters.AddWithValue("@Topic", topic);
        ...
        private void DeleteRecord(int id)
        {
            using (SqlConnection con = new SqlConnection(strcon))
            {
                string query = "DELETE FROM bit_sve WHERE id = @Id";
                using (SqlCommand cmd = new SqlCommand(query, con))
                {
                    cmd.Parameters.AddWithValue("@Id", id);
                    con.Open();
