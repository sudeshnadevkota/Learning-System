...
namespace Learning_System.Mcs_Notes
{
    public partial class digital_marketing : System.Web.UI.Page
    {
        ...
        protected void DownloadFile(object sender, EventArgs e)
        {
            ...
            {
                using (SqlCommand cmd = new SqlCommand("SELECT id, Topic, Name, Data, ContentType FROM mcs_dm WHERE Id=@Id", con))
                {
        ...
        private void BindGrid()
        {
            ...
            using (SqlConnection con = new SqlConnection(constr))
            {
                using (SqlCommand cmd = new SqlCommand("SELECT Id, Topic, Name FROM mcs_dm", con))
                {
                    con.Open();
