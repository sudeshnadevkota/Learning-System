...
namespace Learning_System.Mcs_Notes
{
    public partial class Software_Project_Management : System.Web.UI.Page
    {
        ...
        protected void DownloadFile(object sender, EventArgs e)
        {
            ...
            {
                using (SqlCommand cmd = new SqlCommand("SELECT id, Topic, Name, Data, ContentType FROM MCS_Spm WHERE Id=@Id", con))
                {
        ...
        private void BindGrid()
        {
            ...
            using (SqlConnection con = new SqlConnection(constr))
            {
                using (SqlCommand cmd = new SqlCommand("SELECT Id, Topic, Name FROM MCS_Spm", con))
                {
                    con.Open();
