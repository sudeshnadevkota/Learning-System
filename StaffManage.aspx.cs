    ...
    public partial class StaffManage : System.Web.UI.Page
    {
        ...
        private void LoadDepartmentsDropdown()
        {
            using (SqlConnection con = new SqlConnection(constr))
            {
                ...
                // so they can't reassign a staff member out of their scope.
        ...
        // Looks up the staff member's current DepartmentId so we can verify a DepartmentAdmin
        ...
        protected void btnSaveEdit_Click(object sender, EventArgs e)
        {
            ...
            using (SqlConnection con = new SqlConnection(constr))
            {
                ...
                // Re-check both the staff member's current department and the target department,
                // so a DepartmentAdmin can't move staff into/out of a department they don't own.
                int currentDeptId;
                if (!TryGetStaffDepartment(con, profileId, out currentDeptId))
                {
                    ShowError("Staff member not found.");
