...
namespace Learning_System.Mba_Notes
{
    public partial class marketing_management : System.Web.UI.Page
    {
        ...
        protected void DownloadFile(object sender, EventArgs e)
        {
            ...
            {
                using (SqlCommand cmd = new SqlCommand("SELECT Name, Data, ContentType FROM mba_mm WHERE Id = @Id", con))
                {
        ...
        private void BindGrid()
        {
            ...
            using (SqlConnection con = new SqlConnection(constr))
            {
                using (SqlCommand cmd = new SqlCommand("SELECT Id, Topic, Name FROM mba_mm", con))
                {
                    con.Open();
