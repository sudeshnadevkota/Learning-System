...
namespace Learning_System.Mba_Notes
{
    public partial class international_business : System.Web.UI.Page
    {
        ...
        protected void DownloadFile(object sender, EventArgs e)
        {
            ...
            {
                using (SqlCommand cmd = new SqlCommand("SELECT Name, Data, ContentType FROM mba_ib WHERE Id = @Id", con))
                {
        ...
        private void BindGrid()
        {
            ...
            using (SqlConnection con = new SqlConnection(constr))
            {
                using (SqlCommand cmd = new SqlCommand("SELECT Id, Topic, Name FROM mba_ib", con))
                {
                    con.Open();
