    ...
    public partial class BCS_5_2021 : System.Web.UI.Page
    {
        ...
        protected void DownloadFile(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    using (SqlCommand cmd = new SqlCommand("SELECT Name, Data, ContentType FROM BCS_5_2021 WHERE Id = @Id", con))
                    {
        ...
        private void BindGrid()
        {
            ...
            using (SqlConnection con = new SqlConnection(constr))
            {
                using (SqlCommand cmd = new SqlCommand("SELECT Id, Name, Topic FROM BCS_5_2021", con))
                {
                    con.Open();
