    ...
    public partial class BCS_4_2018 : System.Web.UI.Page
    {
        ...
        protected void DownloadFile(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    using (SqlCommand cmd = new SqlCommand("SELECT Name, Data, ContentType FROM BCS_4_2018 WHERE Id = @Id", con))
                    {
        ...
        private void BindGrid()
        {
            ...
            using (SqlConnection con = new SqlConnection(constr))
            {
                using (SqlCommand cmd = new SqlCommand("SELECT Id, Name, Topic FROM BCS_4_2018", con))
                {
                    con.Open();
