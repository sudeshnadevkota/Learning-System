    ...
    public partial class BIT_1_2019 : System.Web.UI.Page
    {
        ...
        protected void DownloadFile(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    using (SqlCommand cmd = new SqlCommand("SELECT Name, Data, ContentType FROM BIT_1_2019 WHERE Id = @Id", con))
                    {
        ...
        private void BindGrid()
        {
            ...
            using (SqlConnection con = new SqlConnection(constr))
            {
                using (SqlCommand cmd = new SqlCommand("SELECT Id, Name, Topic FROM BIT_1_2019", con))
                {
                    con.Open();
