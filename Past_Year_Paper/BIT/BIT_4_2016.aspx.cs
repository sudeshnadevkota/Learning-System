    ...
    public partial class BIT_4_2016 : System.Web.UI.Page
    {
        ...
        protected void DownloadFile(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    using (SqlCommand cmd = new SqlCommand("SELECT Name, Data, ContentType FROM BIT_4_2016 WHERE Id = @Id", con))
                    {
        ...
        private void BindGrid()
        {
            ...
            using (SqlConnection con = new SqlConnection(constr))
            {
                using (SqlCommand cmd = new SqlCommand("SELECT Id, Name, Topic FROM BIT_4_2016", con))
                {
                    con.Open();
